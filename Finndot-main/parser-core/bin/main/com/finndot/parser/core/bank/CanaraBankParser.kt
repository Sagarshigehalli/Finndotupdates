...
class CanaraBankParser : BankParser() {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        // NEFT Pattern: by Sender NAME,
        val neftSenderPattern = Regex(
            """by\s+Sender\s+([^,]+?),""",
            RegexOption.IGNORE_CASE
        )
        neftSenderPattern.find(message)?.let { match ->
             return "NEFT by Sender ${cleanMerchantName(match.groupValues[1].trim())}"
        }
        ...
        upiMerchantPattern.find(message)?.let { match ->
            ...
            // Exclude "XXXX7122" (account number) being detected as merchant
            if (isValidMerchantName(merchant) && !merchant.matches(Regex("""X+\d{4}"""))) {
                return merchant
            }
        }
    ...
    override fun extractAccountLast4(message: String): String? {
        // Pattern: credited to XXXX7122
        val creditedToPattern = Regex(
            """credited\s+to\s+(?:account\s+)?(?:[X\*]+)?(\d{3,4})""",
            RegexOption.IGNORE_CASE
        )
    ...
    override fun extractBalance(message: String): BigDecimal? {
        // Pattern: Total Avail.bal INR 1,092.62 or Total Avail. Bal INR 38412.7
        val balancePattern = Regex(
            """(?:Total\s+)?Avail\.?\s*bal\s+INR\s+([\d,]+(?:\.\d{1,2})?)""",
            RegexOption.IGNORE_CASE
        )
