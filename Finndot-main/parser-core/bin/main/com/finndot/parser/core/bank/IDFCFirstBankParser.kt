...
class IDFCFirstBankParser : BankParser() {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Check for salary first (using shared logic)
    ...
    override fun extractAccountLast4(message: String): String? {
        // Pattern: Look for A/C followed by X masking and capture ALL trailing digits, then take last 4
        // Handles variants like:
        // - "Savings A/c XX7519" -> capture 7519
        // - "Your A/C XXXXX937519" -> capture 937519 and then last 4 => 7519
        val acPattern = Regex(
            """(?:A/C|Account)\s+[Xx]*([0-9]{4,})""",
            RegexOption.IGNORE_CASE
        )
        ...
        // Check for Card pattern (e.g. "card ending 1234", "Card XX1234")
        val cardPattern = Regex("""Card\s+(?:ending|no\.?|number|XX+)?\s*(?:XX+)?(\d{4})""", RegexOption.IGNORE_CASE)
        cardPattern.find(message)?.let { match ->
            return match.groupValues[1]
        }
        
        // Do NOT call super.extractAccountLast4(message) as it uses generic patterns 
        // that can match reference numbers (e.g. IMPS Ref no 5474)
        return null
    }
