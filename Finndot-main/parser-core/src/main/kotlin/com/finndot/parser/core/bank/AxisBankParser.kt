...
class AxisBankParser : BankParser() {
    ...
    override fun extractBalance(message: String): BigDecimal? {
        ...
        val axisBalancePatterns = listOf(
            // "Avl bal: INR 10000.00"
            Regex("""Avl\s+bal:?\s*INR\s+([0-9,]+(?:\.\d{2})?)""", RegexOption.IGNORE_CASE),
            // "Available Balance INR 30.16"
            Regex("""Available\s+Balance\s+INR\s+([0-9,]+(?:\.\d{2})?)""", RegexOption.IGNORE_CASE)
        )
        for (pattern in axisBalancePatterns) {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        // Pattern: debited from ... on <MERCHANT> <DATE>
        // Example: INR 1028.00 debited from A/c no. XXxxxxy on RESTAURANT XY 02-12-2025...
        val debitedOnPattern = Regex(
            """debited\s+from\s+.*?\s+on\s+([^0-9]+?)\s+\d{2}-\d{2}-\d{4}""",
            RegexOption.IGNORE_CASE
        )
        debitedOnPattern.find(message)?.let { match ->
            val merchant = cleanMerchantName(match.groupValues[1].trim())
            if (isValidMerchantName(merchant)) {
                return merchant
            }
