# FinnDot Web - SMS Parser Tool

A web-based tool for testing and reporting SMS parsing issues for the FinnDot AI app.
...
1. **Clone the repository**
   ...
   cd finndot-tracker/FinnDot-web/server
   ```
...
docker build -t finndot-web .

# Run the container
docker run -p 8080:8080 -e DATABASE_PASSWORD=your_password FinnDot-web
```

### Using systemd (Linux)

1. Create a service file at `/etc/systemd/system/FinnDot-web.service`:
   ```ini
   [Unit]
   Description=FinnDot Web SMS Parser
   After=network.target

   [Service]
   Type=simple
   User=www-data
   WorkingDirectory=/opt/finndot-web
   Environment="DATABASE_PASSWORD=your_password"
   ExecStart=/usr/bin/java -jar /opt/finndot-web/server-all.jar
   Restart=always
...
2. Start the service:
   ```bash
   sudo systemctl enable finndot-web
   sudo systemctl start finndot-web
   ```
