...
android {
    namespace = "com.anomapro.finndot"
    compileSdk = 35
    ...
    defaultConfig {
        applicationId = "com.anomapro.finndot.prd"
        minSdk = 28
        targetSdk = 35
        versionCode = 13
        versionName = "0.13"
    ...
    productFlavors {
        create("fdroid") {
            ...
            buildConfigField("boolean", "GOOGLE_SIGNIN_AVAILABLE", "false")
        }
        create("standard") {
            dimension = "version"
            isDefault = true
            // Standard flavor includes all architectures (including x86 for emulators)
            buildConfigField("boolean", "GOOGLE_SIGNIN_AVAILABLE", "true")
            // Firebase Web Client ID for Google Sign-In (from Firebase Console > Project Settings > Your apps > Web client ID)
            val localProps = rootProject.file("local.properties")
            val webClientId = if (localProps.exists()) {
                val p = Properties()
                p.load(localProps.inputStream())
                p.getProperty("FIREBASE_WEB_CLIENT_ID", "") ?: ""
            } else ""
...
dependencies {
    ...
    // Firebase Auth & Firestore (only for standard flavor - Google Sign-In)
    "standardImplementation"(platform(libs.firebase.bom))
    "standardImplementation"(libs.firebase.auth)
    "standardImplementation"(libs.firebase.firestore)
    "standardImplementation"(libs.credentials)
    "standardImplementation"(libs.credentials.play.services.auth)
    "standardImplementation"(libs.google.id)
    "standardImplementation"(libs.play.services.auth)
    ...
    // OkHttp for loan service integration
