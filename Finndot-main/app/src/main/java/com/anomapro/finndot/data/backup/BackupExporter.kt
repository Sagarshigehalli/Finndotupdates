...
class BackupExporter @Inject constructor(
     ...
     */
    private suspend fun createBackup(privacy: ExportPrivacy): FinndotBackup {
        ...
        val finalTransactions = when (privacy) {
            ExportPrivacy.FULL -> {
                // Ensure SMS data is preserved in full export
                transactions.map { transaction ->
                    transaction.copy(
                        smsBody = transaction.smsBody, // Explicitly preserve SMS body
                        smsSender = transaction.smsSender // Explicitly preserve SMS sender
                    )
                }
            }
            ExportPrivacy.MASKED -> transactions.map { it.copy(
                smsBody = "[REDACTED]",
                smsSender = it.smsSender, // Keep sender even when body is masked
            ...
            ExportPrivacy.ANONYMOUS -> transactions.map { it.copy(
                ...
                description = null,
                smsBody = "[REDACTED]",
                smsSender = "[REDACTED]", // Also mask sender in anonymous mode
                accountNumber = "****"
            )}
        }
        
        return FinndotBackup(
