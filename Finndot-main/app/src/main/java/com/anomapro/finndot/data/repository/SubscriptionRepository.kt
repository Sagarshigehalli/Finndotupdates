...
import com.finndot.parser.core.MandateInfo
...
class SubscriptionRepository @Inject constructor(
    ...
    fun getSubscriptionsDueThisMonth(yearMonth: YearMonth, currency: String? = null): Flow<List<SubscriptionEntity>> {
        val startDate = yearMonth.atDay(1)
        val endDate = yearMonth.atEndOfMonth()
        return subscriptionDao.getSubscriptionsDueInRange(startDate, endDate).map { subs ->
            if (currency != null) subs.filter { it.currency == currency } else subs
        }
    }
