...
import com.anomapro.finndot.ui.components.CategoryChip
...
private fun TransactionDetailContent(
    ...
    Column(
        ...
        if (isEditMode) {
            ...
            ExtractedInfoCard(transaction, viewModel)
        }
...
@Composable
private fun ExtractedInfoCard(transaction: TransactionEntity, viewModel: TransactionDetailViewModel) {
    FinndotCard(
        modifier = Modifier.fillMaxWidth()
    ) {
        Column(
            ...
            // Regret button (only for expenses and credit)
            if (transaction.transactionType in listOf(TransactionType.EXPENSE, TransactionType.CREDIT)) {
                Spacer(modifier = Modifier.height(Spacing.md))
                HorizontalDivider()
                Spacer(modifier = Modifier.height(Spacing.md))
                Row(
                    ...
                    Row(
                        ...
                        Text(
                            text = if (transaction.isRegret) "Marked as Regret Spend" else "Mark as Regret Spend",
                            style = MaterialTheme.typography.bodyMedium,
                            fontWeight = if (transaction.isRegret) FontWeight.Medium else FontWeight.Normal
                        )
                    }
                    FilledTonalButton(
                        onClick = { viewModel.toggleRegretStatus(!transaction.isRegret) }
                    ) {
                        Text(if (transaction.isRegret) "Remove" else "Mark")
                    }
                }
            }
        }
