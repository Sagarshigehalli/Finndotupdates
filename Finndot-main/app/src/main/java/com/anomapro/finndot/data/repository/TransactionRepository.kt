...
class TransactionRepository @Inject constructor(
    ...
    private val regretAutoTagger: RegretAutoTagger
) {
    ...
    suspend fun insertTransaction(transaction: TransactionEntity): Long {
        val tagged = applyRegretAutoTag(transaction)
        return transactionDao.insertTransaction(tagged)
    }
    
    suspend fun insertTransactions(transactions: List<TransactionEntity>) {
        val tagged = transactions.map { applyRegretAutoTag(it) }
        transactionDao.insertTransactions(tagged)
    }
    
    private fun applyRegretAutoTag(transaction: TransactionEntity): TransactionEntity {
        val shouldTag = regretAutoTagger.shouldAutoTagAsRegret(transaction)
        return if (shouldTag && !transaction.isRegret) {
            transaction.copy(isRegret = true)
        } else {
            transaction
        }
    }
