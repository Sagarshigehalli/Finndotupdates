package com.anomapro.finndot.data.database
...
import com.anomapro.finndot.data.database.converter.Converters
...
import com.anomapro.finndot.data.database.entity.ChatMessage
import com.anomapro.finndot.data.database.entity.ExchangeRateEntity
import com.anomapro.finndot.data.database.entity.MerchantMappingEntity
import com.anomapro.finndot.data.database.entity.MerchantNameMappingEntity
import com.anomapro.finndot.data.database.entity.RuleApplicationEntity
import com.anomapro.finndot.data.database.entity.RuleEntity
 ...
 */
@Database(
    entities = [TransactionEntity::class, SubscriptionEntity::class, ChatMessage::class, MerchantMappingEntity::class, MerchantNameMappingEntity::class, CategoryEntity::class, AccountBalanceEntity::class, UnrecognizedSmsEntity::class, CardEntity::class, RuleEntity::class, RuleApplicationEntity::class, ExchangeRateEntity::class, BudgetEntity::class],
    version = 30,
    exportSchema = true,
    autoMigrations = [
        ...
        // Note: v29 to v30 uses manual migration to add normalized_merchant_name column and merchant_name_mappings table
    ]
)
@TypeConverters(Converters::class)
abstract class FinndotDatabase : RoomDatabase() {
    ...
    companion object {
        const val DATABASE_NAME = "finndot_database"
        
        @Volatile
        private var INSTANCE: FinndotDatabase? = null
         ...
         */
        fun getInstance(context: android.content.Context): FinndotDatabase {
         ...
         */
        fun setInstance(database: FinndotDatabase) {
            INSTANCE = database
        }
         ...
         * Adds normalized_merchant_name column to transactions table and creates merchant_name_mappings table.
         */
        val MIGRATION_29_30 = object : Migration(29, 30) {
            override fun migrate(db: SupportSQLiteDatabase) {
                // Add normalized_merchant_name column to transactions table
                db.execSQL("ALTER TABLE transactions ADD COLUMN normalized_merchant_name TEXT")
                
                // Create merchant_name_mappings table
                db.execSQL("""
                ...
                // Create indices for merchant_name_mappings
                db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_merchant_name_mappings_original_name ON merchant_name_mappings (original_name)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_merchant_name_mappings_normalized_name ON merchant_name_mappings (normalized_name)")
            }
        }
    }
