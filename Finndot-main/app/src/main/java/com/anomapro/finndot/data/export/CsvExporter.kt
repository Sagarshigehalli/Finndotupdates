package com.anomapro.finndot.data.export
...
class CsvExporter @Inject constructor(
    ...
    /**
     * Exports SMS data to CSV file, including:
     * 1. All transactions that have SMS data (smsBody or smsSender)
     * 2. All unrecognized SMS messages
     * 
     * @param transactionsWithSms List of transactions that have SMS data
     * @param unrecognizedSms List of unrecognized SMS messages
     * @param fileName Optional custom filename (without extension)
     * @return Flow emitting progress updates and final Uri
     */
    fun exportSmsData(
        transactionsWithSms: List<TransactionEntity>,
        unrecognizedSms: List<UnrecognizedSmsEntity>,
        fileName: String? = null
    ): Flow<ExportResult> = flow {
        emit(ExportResult.Progress(0f, "Preparing SMS export..."))
        ...
        try {
            ...
            // Generate filename
            val timestamp = LocalDateTime.now().format(fileNameFormatter)
            val finalFileName = fileName ?: "sms_database_$timestamp"
            val file = File(exportDir, "$finalFileName.csv")
            
            emit(ExportResult.Progress(0.1f, "Creating CSV file..."))
            
            val totalItems = transactionsWithSms.size + unrecognizedSms.size
            ...
            FileWriter(file).use { fileWriter ->
                CSVWriter(fileWriter).use { csvWriter ->
                    ...
                    emit(ExportResult.Progress(0.2f, "Exporting transaction SMS data..."))
                    
                    // Write transactions with SMS data
                    transactionsWithSms.forEachIndexed { index, transaction ->
                        csvWriter.writeNext(arrayOf(
                            "Transaction",
                            transaction.dateTime.format(dateFormatter),
                            transaction.dateTime.format(timeFormatter),
                        ...
                        processedItems++
                        // Update progress (20% to 70% for transactions)
                        if (transactionsWithSms.isNotEmpty()) {
                            val progress = 0.2f + (0.5f * (index + 1) / transactionsWithSms.size)
                            if (index % 100 == 0 || index == transactionsWithSms.size - 1) {
                                emit(ExportResult.Progress(
                                    progress,
                                    "Exporting transaction SMS ${index + 1} of ${transactionsWithSms.size}..."
                                ))
                            }
                        }
                    }
                    
                    emit(ExportResult.Progress(0.7f, "Exporting unrecognized SMS data..."))
                    
                    // Write unrecognized SMS messages
                    unrecognizedSms.forEachIndexed { index, sms ->
                        csvWriter.writeNext(arrayOf(
                            "Unrecognized",
                            sms.receivedAt.format(dateFormatter),
                            sms.receivedAt.format(timeFormatter),
                            sms.sender,
                            sms.smsBody,
                            if (sms.reported) "Reported" else "Unreported",
                        ...
                        processedItems++
                        // Update progress (70% to 95% for unrecognized SMS)
                        if (unrecognizedSms.isNotEmpty()) {
                            val progress = 0.7f + (0.25f * (index + 1) / unrecognizedSms.size)
                            if (index % 100 == 0 || index == unrecognizedSms.size - 1) {
                                emit(ExportResult.Progress(
                                    progress,
                                    "Exporting unrecognized SMS ${index + 1} of ${unrecognizedSms.size}..."
                                ))
                            }
                        }
                    }
            ...
            emit(ExportResult.Error(
                message = "Failed to export SMS data: ${e.message}",
                exception = e
            ))
        }
    }.flowOn(Dispatchers.IO)
    ...
    /**
     * Exports SMS data directly to file (blocking version for simple use cases)
     */
    suspend fun exportSmsDataToFile(
        transactionsWithSms: List<TransactionEntity>,
        unrecognizedSms: List<UnrecognizedSmsEntity>,
        fileName: String? = null
    ): Uri = withContext(Dispatchers.IO) {
        ...
        FileWriter(file).use { fileWriter ->
            CSVWriter(fileWriter).use { csvWriter ->
                ...
                // Write transactions with SMS data
                transactionsWithSms.forEach { transaction ->
                    csvWriter.writeNext(arrayOf(
                        "Transaction",
                        transaction.dateTime.format(dateFormatter),
                        transaction.dateTime.format(timeFormatter),
