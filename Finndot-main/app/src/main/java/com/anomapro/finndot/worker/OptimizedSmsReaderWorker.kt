...
import com.finndot.parser.core.bank.IndianBankParser
import com.finndot.parser.core.bank.SBIBankParser
import com.finndot.parser.core.bank.IndusIndBankParser
...
import com.anomapro.finndot.data.mapper.toEntityType
    ...
    for (sms in messages) {
        try {
            ...
            if (parser != null) {
                ...
                // Try LLM parsing for unrecognized SMS
                val llmParsedTransaction = llmSmsParser.parse(sms.body, sms.sender, sms.timestamp)
                
                if (llmParsedTransaction != null) {
                    Log.d(TAG, "LLM successfully parsed SMS from ${sms.sender}")
                    parsedCount++
                    
                    val saveResult = saveParsedTransaction(llmParsedTransaction, sms)
                    if (saveResult) savedCount++
                } else {
        ...
        for (sms in batch) {
            try {
                ...
                if (llmParsedTransaction != null) {
                    ...
                    val success = saveParsedTransaction(llmParsedTransaction, sms)
                    if (success) {
                        savedCount++
...
private suspend fun processBalanceUpdate(
    ...
    if (parsedTransaction.accountLast4 != null) {
        ...
        val targetAccountLast4 = if (isFromCard) {
            ...
            val result = when {
                card.cardType == com.anomapro.finndot.data.database.entity.CardType.CREDIT -> {
                    // Credit cards get balance entries
                    Log.d(TAG, "CREDIT card - will create balance entry")
                    parsedTransaction.accountLast4
                }

                card.cardType == com.anomapro.finndot.data.database.entity.CardType.DEBIT && card.accountLast4 != null -> {
        ...
        if (targetAccountLast4 != null) {
            val isCreditCard = (parsedTransaction.type.toEntityType() == TransactionType.CREDIT) ||
                    ...
                    } == com.anomapro.finndot.data.database.entity.CardType.CREDIT
...
private suspend fun readSmsMessages(): List<SmsMessage> {
    ...
    try {
        ...
        // Update scan tracking only when we found messages - otherwise next scan retries full range
        if (messages.isNotEmpty()) {
            ...
            Log.w(TAG, "No SMS found - lastScanTimestamp not updated so next scan will retry full range")
        }
