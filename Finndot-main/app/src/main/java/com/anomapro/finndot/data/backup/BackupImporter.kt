package com.anomapro.finndot.data.backup
...
import com.anomapro.finndot.data.database.entity.*
...
class BackupImporter @Inject constructor(
    ...
    private val database: FinndotDatabase,
     ...
     */
    private suspend fun readBackupFile(uri: Uri): FinndotBackup {
        return withContext(Dispatchers.IO) {
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                val reader = BufferedReader(InputStreamReader(inputStream))
                val content = reader.readText()
                gson.fromJson(content, FinndotBackup::class.java)
     ...
     */
    private fun isCompatibleVersion(backup: FinndotBackup): Boolean {
        // For now, accept all v1.x backups
        return backup.format.startsWith("FinnDot Backup v1")
    }
    
    /**
     * Clear all app data (transactions, categories, etc.) from local database.
     * Does not clear user preferences or auth state.
     */
    suspend fun clearAllData(): Result<Unit> = runCatching {
        database.withTransaction {
            ...
            database.merchantNameMappingDao().deleteAllMappings()
     ...
     */
    private suspend fun replaceAllData(backup: FinndotBackup): ImportResult {
     ...
     */
    private suspend fun mergeData(backup: FinndotBackup): ImportResult {
        var importedTransactions = 0
        var importedCategories = 0
    ...
    private suspend fun importPreferences(preferences: PreferencesSnapshot) {
        ...
        userPreferencesRepository.setOnboardingCompleted(preferences.app.hasCompletedOnboarding)
