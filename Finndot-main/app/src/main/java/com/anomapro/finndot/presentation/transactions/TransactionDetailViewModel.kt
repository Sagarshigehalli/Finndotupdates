...
class TransactionDetailViewModel @Inject constructor(
    ...
    private fun determinePrimaryCurrency(transaction: TransactionEntity) {
        val bankName = transaction.bankName
        val primaryCurrency = if (!bankName.isNullOrEmpty()) {
            com.anomapro.finndot.utils.CurrencyFormatter.getBankBaseCurrency(bankName)
        } else {
    ...
    fun toggleRegretStatus(isRegret: Boolean) {
        val transaction = _transaction.value ?: return
        viewModelScope.launch {
            try {
                transactionRepository.updateRegretStatus(transaction.id, isRegret)
                _transaction.value = transaction.copy(isRegret = isRegret)
            } catch (e: Exception) {
                _errorMessage.value = "Failed to update regret status"
            }
        }
    }
