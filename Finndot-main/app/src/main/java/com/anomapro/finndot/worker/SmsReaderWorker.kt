                            ...
                            if (rowId != -1L) {
                                ...
                                val targetAccountLast4 = if (isFromCard) {
                                    ...
                                    if (card == null) {
                                        ...
                                        // Determine if it's a credit or debit card based on SMS content first
                                        val lowerBody = sms.body.lowercase()
                                        val isCredit = when {
                                            lowerBody.contains("credit card") -> true
                                            lowerBody.contains("debit card") -> false
                                            // Heuristic: presence of available limit typically indicates credit card
                                            parsedTransaction.creditLimit != null -> true
                                            else -> (parsedTransaction.type.toEntityType() == TransactionType.CREDIT)
                                        }
