...
import com.anomapro.finndot.data.model.ChatContext
...
import com.anomapro.finndot.utils.CurrencyUtils
...
class LlmRepository @Inject constructor(
    ...
    /**
     * One-shot AI advice for analytics - does not use chat history.
     * Use when model is downloaded to get savings tips based on spending data.
     */
    fun getAnalyticsSavingsAdvice(analyticsContext: String): Flow<String> = flow {
        val currentModelState = modelRepository.modelState.first()
        if (currentModelState == ModelState.DOWNLOADING) {
            throw Exception("Model is downloading. Please wait.")
        }
        if (!modelRepository.isModelDownloaded()) {
            throw Exception("Model not downloaded. Download from Settings for AI advice.")
        }
        if (!llmService.isInitialized()) {
            val modelFile = modelRepository.getModelFile()
            if (!modelFile.exists()) {
                throw Exception("Model file not found. Download from Settings.")
            }
            val initResult = llmService.initialize(modelFile.absolutePath)
            if (initResult.isFailure) {
                throw initResult.exceptionOrNull() ?: Exception("Failed to initialize AI")
            }
        }
        val prompt = """
        You are FinnDot AI, a financial coach focused on helping people find where they lose money and how to save. Based on this data, identify the top 2-4 MONEY LEAKS and give specific, actionable steps to fix each. Be direct: "You're overspending on X - do Y to save Z." Use plain text only - no markdown, asterisks, or special formatting. Keep each tip to 1-2 sentences. Focus on the biggest impact first.

        Spending data:
        $analyticsContext

        User: Where am I losing money? How can I save?
        Assistant:""".trimIndent()
        llmService.generateResponseStream(prompt).collect { emit(it) }
    }
    ...
    private fun buildSystemPrompt(context: ChatContext): String {
        ...
        return """
        You are FinnDot AI, a friendly financial assistant helping users track expenses and manage money.
    ...
    suspend fun getFormattedContextForDisplay(): String {
        ...
        return """
        Hi! I'm FinnDot AI, your financial assistant.
